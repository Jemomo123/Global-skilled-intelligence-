...
def scan_new_zealand(db: Session) -> int:
    country = "New Zealand"
    sources = [
        {"name": "Global Trade Matrix Ingest Alpha", "url": "https://ec.europa.eu/eures/eures-apps/services/rss/jobs?keyword=pipefitter"},
        {"name": "Global Trade Matrix Ingest Beta", "url": "https://ec.europa.eu/eures/eures-apps/services/rss/jobs?keyword=gasfitter"}
    ]
    total_new_jobs = 0
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=15) as response:
                ...
                for item in root.findall('.//item'):
                    ...
                    title = item.find('title').text if item.find('title') is not None else 'Commercial Gasfitter'
                    ...
                    db.add(Job(
                        title=title,
                        company="Tasman Mechanical Engineering Ltd",
                        country=country,
                        city="Auckland",
                        salary="$40 - $55 / hr NZD",
