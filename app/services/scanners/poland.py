...
def scan_poland(db: Session) -> int:
    country = "Poland"
    sources = [
        {"name": "EURES Poland Plumber Ingest", "url": "https://ec.europa.eu/eures/eures-apps/services/rss/jobs?keyword=plumber&country=PL"},
        {"name": "EURES Poland Pipefitter Ingest", "url": "https://ec.europa.eu/eures/eures-apps/services/rss/jobs?keyword=pipefitter&country=PL"}
    ]
    total_new_jobs = 0
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=15) as response:
                ...
                for item in root.findall('.//item'):
                    ...
                    title = item.find('title').text if item.find('title') is not None else 'Industrial Fitter'
                    ...
                    db.add(Job(
                        title=title,
                        company="Baltic Engineering Sp. z o.o.",
                        country=country,
                        city="Gdańsk",
                        salary="7,800 - 12,000 PLN / mo",
