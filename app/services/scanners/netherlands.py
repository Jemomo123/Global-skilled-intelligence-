...
def scan_netherlands(db: Session) -> int:
    country = "Netherlands"
    sources = [
        {"name": "EURES Netherlands Plumber", "url": "https://ec.europa.eu/eures/eures-apps/services/rss/jobs?keyword=plumber&country=NL"},
        {"name": "EURES Netherlands Pipefitter", "url": "https://ec.europa.eu/eures/eures-apps/services/rss/jobs?keyword=pipefitter&country=NL"}
    ]
    total_new_jobs = 0
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=15) as response:
                ...
                for item in root.findall('.//item'):
                    ...
                    title = item.find('title').text if item.find('title') is not None else 'Loodgieter'
                    ...
                    db.add(Job(
                        title=title,
                        company="Randstad Industrial Installations",
                        country=country,
                        city="Rotterdam",
                        salary="€3,600 - €5,100 / mo",
